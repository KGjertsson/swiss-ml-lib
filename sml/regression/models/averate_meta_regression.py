...
class StackingAveragedModels(BaseEstimator, RegressorMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        k_fold = KFold(n_splits=self.n_folds, shuffle=True, random_state=156)

        # Train cloned base models then create out-of-fold predictions
        # that are needed to train the cloned meta-model
        out_of_fold_predictions = np.zeros((X.shape[0], len(self.base_models)))
        for i, model in enumerate(self.base_models):
            for train_index, holdout_index in k_fold.split(X, y):
    ...
    # averaged predictions as meta-features for the final prediction which is
    # done by the meta-model
    def predict(self, X):
